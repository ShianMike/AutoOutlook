...
import type { SpcVerificationSummary } from '../types/outlookArtifacts';
...
export default function VerificationPanel({ spcVerification }: VerificationPanelProps) {
  const [showDetails, setShowDetails] = useState(false);
  const agreementFraction = normalizeFraction(spcVerification?.agreementFraction);

  if (!spcVerification || agreementFraction === null) {
    return <MissingVerificationPanel spcVerification={spcVerification} />;
  }
  ...
  return (
    ...
    >
      <div className="grid gap-4">
        ...
        <div className="grid grid-cols-1 items-stretch gap-4 xl:grid-cols-[minmax(280px,0.95fr)_minmax(320px,1fr)_minmax(360px,1.05fr)]">
                ...
                <div className="mt-2 grid gap-3 sm:grid-cols-[auto_minmax(0,1fr)] sm:items-center xl:grid-cols-1 2xl:grid-cols-[auto_minmax(0,1fr)]">
                  ...
                  <div className="border-l-0 border-ink pl-0 sm:border-l-[3px] sm:pl-3 xl:border-l-0 xl:pl-0 2xl:border-l-[3px] 2xl:pl-3">
                    ...
                    <p className="mt-2 font-mono text-[10px] font-black uppercase tracking-[0.2em] text-ink/50">
                      {formatCount(spcVerification.agreementCells)} aligned / {formatCount(spcVerification.comparisonGridCells)} evaluated
                    </p>
                  </div>
                </div>
              </div>
          ...
          >
            ...
            <CategoryLedger
              predicted={spcVerification.predictedCategories}
              official={spcVerification.officialCategories}
            />
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-1 gap-3 border-[3px] border-ink bg-paper p-3 shadow-retro-sm lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
        <div>
          ...
          <p className="mt-1 font-mono text-[10px] uppercase tracking-[0.16em] text-ink/65">
            {spcVerification.leakageGuard || 'Official SPC outlook is used only after prediction artifacts are generated.'}
          </p>
        </div>
        <button
          type="button"
      ...
      {showDetails && (
        <DiagnosticLog spcVerification={spcVerification} />
      )}
    </RetroPanel>
  );
}
