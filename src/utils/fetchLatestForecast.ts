...
import type { ActiveRegion, ForecastBundle, ForecastProvider } from '../types/forecast';
...
async function tryProvider(p: ForecastProvider, activeRegion?: ActiveRegion): Promise<ForecastBundle> {
  const ctrl = new AbortController();
  const timeoutMs = p.id === 'backend' ? BACKEND_TIMEOUT_MS : PROVIDER_TIMEOUT_MS;
  return withTimeout(p.fetchBundle(ctrl.signal, activeRegion), timeoutMs, ctrl);
}
 ...
 */
export async function fetchLatestForecast(activeRegion?: ActiveRegion): Promise<FetchResult> {
  ...
  for (const provider of chain) {
    try {
      const bundle = await tryProvider(provider, activeRegion);
