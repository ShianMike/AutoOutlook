...
# Catalog assets are emitted as static JSON (index + one file per event) so the
# frontend lazy-loads them instead of bundling ~58 MB into the JS.
OUTPUT_PATH = PROJECT_ROOT / "public" / "data" / "enh-plus-archive"

# Lightweight metadata fields kept in index.json (no geojson payloads).
...
def main() -> None:
    ...
    write_json_assets(output_path, events)
    print(f"Wrote {output_path.relative_to(PROJECT_ROOT)} index + {len(events)} per-event file(s).")
...
def read_existing_events(output_path: Path) -> list[dict[str, Any]]:
    """Load the full per-event catalog objects from the static JSON assets."""
    index_path = output_path / "index.json"
    if not index_path.exists():
        raise SystemExit(
            f"{index_path} does not exist; run a full generate before --reports-only."
        )
    index = read_json(index_path)
    if not isinstance(index, list):
        raise SystemExit(f"Catalog index in {index_path} is not a JSON array.")
    events: list[dict[str, Any]] = []
    for item in index:
        event_date = item.get("eventDate") if isinstance(item, Mapping) else None
        if not event_date:
            continue
        event_path = output_path / f"{event_date}.json"
...
def write_json_assets(output_dir: Path, events: list[dict[str, Any]]) -> None:
    """Write the static catalog assets: a lightweight ``index.json`` plus one
    ``<eventDate>.json`` per event. The frontend lazy-loads these instead of
    bundling the full payloads into the JS."""
    output_dir.mkdir(parents=True, exist_ok=True)
    index: list[dict[str, Any]] = []
    for event in events:
        event_date = str(event.get("eventDate"))
        (output_dir / f"{event_date}.json").write_text(
            json.dumps(event, ensure_ascii=True, default=json_default),
            encoding="utf-8",
            newline="\n",
        )
        index.append({field: event.get(field) for field in CATALOG_INDEX_FIELDS})
    (output_dir / "index.json").write_text(
        json.dumps(index, indent=2, ensure_ascii=True, default=json_default),
        encoding="utf-8",
        newline="\n",
    )
