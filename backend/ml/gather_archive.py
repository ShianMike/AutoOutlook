...
DEFAULT_OUTPUT = Path(__file__).resolve().parents[1] / "ml_data" / "archive_samples.parquet"
...
class HrrrSampleRef:
    ...
    def valid_time(self) -> datetime:
        cycle = datetime.strptime(f"{self.run_date}{self.run_cycle:02d}", "%Y%m%d%H").replace(tzinfo=timezone.utc)
...
def discover_spc_csv_urls(session: requests.Session) -> dict[str, list[str]]:
    ...
    for href in re.findall(r'href=["\']([^"\']+\.csv)["\']', response.text, flags=re.IGNORECASE):
        lower = href.lower()
        hazard = None
        if "torn" in lower:
            hazard = "tornado"
...
def selected_spc_csv_urls(discovered: dict[str, list[str]], years: Iterable[int]) -> dict[str, list[str]]:
    year_set = set(int(y) for y in years)
    selected: dict[str, list[str]] = {}
    for hazard, urls in discovered.items():
        annual_urls: list[str] = []
        for year in sorted(year_set):
            annual_urls.extend([url for url in urls if url.rsplit("/", 1)[-1].startswith(f"{year}_")])
...
def _parse_spc_datetime(row: Any, year_col: str, month_col: str, day_col: str, time_col: str) -> datetime | None:
    try:
...
def _normalize_spc_frame(frame: Any, hazard: str, years: set[int], months: set[int]) -> list[dict[str, Any]]:
    ...
    for _, row in frame.iterrows():
        ...
        if report_time is None or report_time.year not in years or report_time.month not in months:
            continue
        try:
...
def load_spc_reports(session: requests.Session, years: Iterable[int], months: Iterable[int]) -> list[dict[str, Any]]:
    pd = _require_frame_deps()
    year_set = set(int(y) for y in years)
    month_set = set(int(m) for m in months)
    reports: list[dict[str, Any]] = []
    selected_urls_by_hazard = selected_spc_csv_urls(discover_spc_csv_urls(session), year_set)
...
def _run_dates_for_window(years: Iterable[int], months: Iterable[int], run_dates: Iterable[str] | None) -> Iterable[str]:
    if run_dates:
    ...
    for run_date in _run_dates_for_window(years, months, run_dates):
        for cycle in cycles:
            for forecast_hour in forecast_hours:
                ...
                if valid_time_filter is not None and ref.valid_time not in valid_time_filter:
                    continue
                yield ref
...
def _selected_ranges(records: list[tuple[int, int, str]], content_length: int | None) -> list[tuple[int, int]]:
...
def _fetch_hrrr_subset(session: requests.Session, ref: HrrrSampleRef) -> tuple[np.ndarray, np.ndarray, dict[str, np.ndarray]]:
    ...
    content_length = int(head.headers["content-length"]) if head.ok and head.headers.get("content-length") else None
    ...
    def _fetch_range(start_end: tuple[int, int]) -> tuple[int, bytes]:
        start, end = start_end
        r = session.get(ref.grib_url, headers={"Range": f"bytes={start}-{end}"}, timeout=60)
        r.raise_for_status()
...
def _value(fields: dict[str, np.ndarray], key: str, i_lat: int, i_lon: int, default: float = 0.0) -> float:
...
def _ingredients_at(
    ...
    mlcape = _value(fields, "cape_ml", i_lat, i_lon, cape * 0.85)
    mucape = _value(fields, "cape_mu", i_lat, i_lon, max(cape, mlcape))
    cin = _value(fields, "cin_ml", i_lat, i_lon, _value(fields, "cin", i_lat, i_lon))
    ...
    if all(k in fields for k in ("u500", "v500", "u10", "v10")):
        shear_kt = float(np.hypot(
            _value(fields, "u500", i_lat, i_lon) - _value(fields, "u10", i_lat, i_lon),
            _value(fields, "v500", i_lat, i_lon) - _value(fields, "v10", i_lat, i_lon),
        ) * 1.9438445)
    else:
        shear_kt = 0.0
...
def _dedupe_points(points: Iterable[tuple[float, float]], max_points: int) -> list[tuple[float, float]]:
...
def _candidate_points(
    ...
    if negative_points_per_hour > 0:
        ...
        return _dedupe_points(
            [*negative_points[:negative_count], *report_points, *negative_points[negative_count:]],
            max_points,
        )
...
def _dedupe_feature_label_rows(rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], int]:
...
def gather(args: argparse.Namespace) -> int:
    ...
    if args.dry_run:
        selected_urls_by_hazard = selected_spc_csv_urls(discover_spc_csv_urls(session), args.years)
        first_refs = list(iter_hrrr_refs(
            args.years,
            args.months,
            args.cycles,
            args.forecast_hours,
        ...
        print(json.dumps({
            ...
            "firstHrrrRefs": [ref.__dict__ | {"validTimeISO": ref.valid_time.isoformat()} for ref in first_refs],
    ...
    def _checkpoint(current_rows: list[dict[str, Any]], _args: argparse.Namespace) -> None:
        ...
        print(f"  [checkpoint] {len(current_rows)} rows → {ckpt_path.name} (resume: --start-from {tag})", flush=True)
    ...
    def _process_ref(ref: HrrrSampleRef) -> list[dict[str, Any]]:
        ...
        worker_rng = random.Random(args.random_seed ^ hash(ref.run_date) ^ ref.run_cycle ^ ref.forecast_hour)
        try:
        ...
        for lat, lon in _candidate_points(
            ...
            labels = labels_for_sample(reports, ref.valid_time, lat, lon, args.radius_km)
    ...
    if args.start_from:
        ...
        if not m:
            raise ValueError(f"--start-from must be YYYYMMDD_zHH_+FFh, got: {args.start_from}")
        sf_date, sf_cycle, sf_fh = m.group(1), int(m.group(2)), int(m.group(3))
        skip_idx = next(
            (i for i, r in enumerate(all_refs)
             if r.run_date == sf_date and r.run_cycle == sf_cycle and r.forecast_hour == sf_fh),
            None,
        )
        if skip_idx is not None:
            all_refs = all_refs[skip_idx + 1:]
            print(f"Resuming after {args.start_from} — {len(all_refs)} refs remaining", flush=True)
        ...
        for fut in as_completed(future_to_ref):
            ...
            if batch and batch[0].get("__skip__"):
                ...
                if fetched_count[0] % 25 == 0 or fetched_count[0] == 1:
                    print(f"[{fetched_count[0]} fetched | {len(rows)} rows | {skipped_count[0]} skipped] last: {ref.run_date} z{ref.run_cycle:02d} +{ref.forecast_hour:02d}h", flush=True)
    ...
    if args.start_from:
        ...
        if base_path.exists():
            ...
            print(f"Merged {len(existing_df)} existing rows + {len(rows)} new rows = {len(new_df)} total", flush=True)
    ...
    print(json.dumps({
        ...
        "uniqueRunDates": len({str(row.get("runDate", "")) for row in final_rows}),
        "positives": {hazard: int(sum(int(row.get(f"label_{hazard}", 0)) for row in final_rows)) for hazard in HAZARD_KEYS},
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--forecast-hours", type=int, nargs="+", default=list(PILOT_FORECAST_HOURS))
    parser.add_argument("--run-dates", nargs="+", help="Optional HRRR run dates to gather, formatted YYYYMMDD")
    parser.add_argument("--valid-hours", nargs="+", help="Optional valid hours to keep, formatted YYYYMMDDHH")
